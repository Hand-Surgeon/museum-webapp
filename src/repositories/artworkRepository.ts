...
function transformArtworkFromDB(dbArtwork: DBArtwork): Artwork {
  return {
    ...
    titleEn: dbArtwork.title_en || undefined,
    ...
    detailedDescription: dbArtwork.detailed_description || undefined,
    historicalBackground: dbArtwork.historical_background || undefined,
    artisticFeatures: dbArtwork.artistic_features || undefined,
    imageUrl: dbArtwork.image_url,
    featured: dbArtwork.featured,
    culturalProperty: dbArtwork.cultural_property || undefined,
    nationalTreasureNumber: String(dbArtwork.national_treasure_number || ''),
    museum: dbArtwork.museum,
    inventoryNumber: dbArtwork.inventory_number,
    era: dbArtwork.era || undefined,
    significance: dbArtwork.significance || undefined,
    displayLocation: dbArtwork.display_location || undefined,
  }
}
...
export class ArtworkRepository implements IArtworkRepository {
  ...
  async search(query: string): Promise<Artwork[]> {
    const { data, error } = await supabase
      ...
      .select('*')
      .or(
        `title.ilike.%${query}%,description.ilike.%${query}%,detailed_description.ilike.%${query}%`
      )
      .order('id', { ascending: true })
  ...
  async findByFilters(filters: ArtworkFilters): Promise<Artwork[]> {
    ...
    if (filters.search) {
      query = query.or(
        `title.ilike.%${filters.search}%,description.ilike.%${filters.search}%,detailed_description.ilike.%${filters.search}%`
      )
    }
